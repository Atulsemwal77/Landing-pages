...
//   etch recent blogs
  const fetchRecentBlogs = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_BACKEND}/api/itBlog`);
  ...
  return (
    <div className="container mx-auto p-4 sm:px-6 lg:px-8 max-w-7xl ">
      {/* Banner */}
      <div className="relative w-full h-64 md:h-96 lg:h-[500px] rounded-2xl overflow-hidden mb-8">
        <img
          src={`${import.meta.env.VITE_BACKEND}/${blog.image.replace(
            /\\/g,
            "/"
          )}`}
          alt={blog.title}
        ...
        <aside className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            ...
            {recentBlogs.slice(0, 3).map((post) => (
              ...
              >
                <img
                  src={`${import.meta.env.VITE_BACKEND}/${post.image}`}
                  alt={post.title}
...
export default ItBlogDetailPage;
